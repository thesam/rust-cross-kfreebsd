...
RUN tar xf llvm-3.8.1.src.tar.xz
WORKDIR /build/build-llvm

#TODO: LLVM does not seem to handle PATH_MAX correctly for kFreeBSD
RUN sed -i "s/_POSIX_PATH_MAX/4096/" /build/llvm/llvm-3.8.1.src/utils/unittest/googletest/src/gtest-filepath.cc
RUN sed -i "s/PATH_MAX/4096/" /build/llvm/llvm-3.8.1.src/tools/dsymutil/DwarfLinker.cpp

RUN mkdir -p /build/build-rust/x86_64-unknown-kfreebsd-gnu/llvm/lib/
RUN ../llvm/llvm-3.8.1.src/configure --host=x86_64-kfreebsd-gnu --target=x86_64-kfreebsd-gnu --prefix=/build/build-rust/x86_64-unknown-kfreebsd-gnu/llvm/
